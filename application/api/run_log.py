    ...
    for table_name in table_name_fix_lst:
        ...
        if batch_id:
            ...
            if len(table_name_fix_lst) == 1 and to_time:
                sql = table.select().where(table.c.create_time.__le__(to_time)).where(table.c.batch_id.in_(batch_list))
                if from_time:
                    sql.where(table.c.create_time.__ge__(from_time))

            elif table_name == table_name_fix_lst[0] and from_time:
                sql = table.select().where(table.c.create_time.__ge__(from_time)).\
                    where(table.c.batch_id.in_(batch_list))

            elif table_name == table_name_fix_lst[-1] and to_time:
                sql = table.select().where(table.c.create_time.__le__(to_time)). \
                    where(table.c.batch_id.in_(batch_list))
...
@table_decorator
def add_use_case_run_log_table(**kwargs):
...
@table_decorator
def add_interface_run_log_table(**kwargs):
