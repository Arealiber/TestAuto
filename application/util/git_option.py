...
def get_new_tag(soft_name, work_path):
    ...
    if int(max_phase_num) < 9:
        new_tag_name = '%s-test-v%d.%d.%d' % (soft_name, max_major_ver_num, max_child_ver_num, max_phase_num+1)
    elif max_child_ver_num < 9:
        new_tag_name = '%s-test-v%d.%d.0' % (soft_name, max_major_ver_num, max_child_ver_num+1)
    else:
        new_tag_name = '%s-test-v%d.0.0' % (soft_name, max_major_ver_num+1)
    return new_tag_name
